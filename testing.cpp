...
void test_JointToAbsoluteVelocities(void) {
    _input_ input = _input_(4);
    VectorXd dq = JOINT_TO_ABSOLUTE_VELOCITIES(input.alpha0, input.dalpha0, input);
...
// jak test sie powiedzie, mozna wsadzic do petli for
    ...
    TEST_CHECK_(diff.norm() <= eps, "error = %f, value = [%f, %f, %f]", diff.norm(), Vref(1), Vref(2), Vref(3));

    i = 1;
    test = dq.segment(3*i, 3);
    Vref = S12_0.transpose() * Vref + input.pickBodyType(i).H * input.dalpha0(i);
    Vector3d diff = test - Vref;
    TEST_CHECK_(diff.norm() <= eps, "error = %f, value = [%f, %f, %f]", diff.norm(), Vref(1), Vref(2), Vref(3));

    i = 2;
    test = dq.segment(3*i, 3);
    Vref = S12_1.transpose() * Vref + input.pickBodyType(i).H * input.dalpha0(i);
    Vector3d diff = test - Vref;
    TEST_CHECK_(diff.norm() <= eps, "error = %f, value = [%f, %f, %f]", diff.norm(), Vref(1), Vref(2), Vref(3));

    i = 3;
    test = dq.segment(3*i, 3);
    Vref = S12_2.transpose() * Vref + input.pickBodyType(i).H * input.dalpha0(i);
    Vector3d diff = test - Vref;
    TEST_CHECK_(diff.norm() <= eps, "error = %f, value = [%f, %f, %f]", diff.norm(), Vref(1), Vref(2), Vref(3));
}
...
void test_SetPJoint(void) {
    ...
    _input_ input = _input_(4);
    VectorXd sigmaStacked = input.GET_PJOINT_SIGMA_SOMEHOW().second;
    VectorXd dq = JOINT_TO_ABSOLUTE_VELOCITIES(input.alpha0, input.dalpha0, input);
    VectorXd alphaAbs = joint2AbsAngles(input.alpha0);
...
TEST_LIST = {
   { "phi", test_Phi },
   { "jacobian", test_Fq },
   { "joint2AbsCoord", test_jointToAbsoluteCoordinates },
   { "joint2AbsVel", test_JointToAbsoluteVelocities },
   { "setPjoint", test_SetPJoint },
