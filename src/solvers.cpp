...
_solutionAdj_ RK_AdjointSolver(const VectorXd& uVec, const _solution_& solutionFwd, const _input_& input) {
	...
	y_m1 = boundaryConditions(solutionFwd, input);

	// backwards integration
	for (int i = 1; i < input.Nsamples; i++) {
        const double t = T(i-1);
		VectorXd k1 = RHS_ADJOINT(t			, y_m1              , uVec, solutionFwd, input, solution);
		VectorXd k2 = RHS_ADJOINT(t + dt/2.0, y_m1 + dt/2.0 * k1, uVec, solutionFwd, input);
		VectorXd k3 = RHS_ADJOINT(t + dt/2.0, y_m1 + dt/2.0 * k2, uVec, solutionFwd, input);
		VectorXd k4 = RHS_ADJOINT(t + dt    , y_m1 + dt     * k3, uVec, solutionFwd, input);

		VectorXd y = y_m1 +  dt/6 * (k1 + 2*k2 + 2*k3 + k4);
		y_m1 = y;
	}

	RHS_ADJOINT(input.Tk, y_m1, uVec, solutionFwd, input, solution); // save last entry
